...
# AlphaGenerator removed - using fixed alpha = 1.0
from .spatial_binding import SpatialBinding
# Feature binding is disabled for spatial variant
...
class MVBA(nn.Module):
    ...
    def __init__(
        ...
        # Skip alpha validation for fixed alpha variant
        ...
        # Fixed alpha value (no enhancement)
        ...
        # 3. AlphaGenerator REMOVED - using fixed alpha = 1.0
        
        # 4. Spatial Binding: Determines WHERE each object is through a query-key mechanism
        # Creates segmentation masks showing which pixels belong to which slot
        self.spatial_binding = SpatialBinding(
            slot_dim=slot_dim,
            feature_dim=self.total_feature_dim,
            query_dim=32,
        ...
        # Spatial variant: Feature binding is disabled
    ...
    def forward(
        ...
        # === Step 3: Generate Fixed Alpha Values ===
        # Create fixed alpha = 1.0 for all slots (no enhancement)
        device = images.device
        alphas = {
            'spatial': torch.ones(B, self.n_slots, 1, device=device) * self.fixed_alpha,
            'feature': torch.ones(B, self.n_slots, 1, device=device) * self.fixed_alpha
        }
        
        # === Step 4: Spatial Binding (WHERE) ===
        # Determine which pixels belong to which object
        # Uses alpha to sharpen the competition between slots
        spatial_attention = self.spatial_binding(slots, features, alphas)
        # spatial_attention: (B, n_slots, H, W) - segmentation masks
        
        # === Spatial Variant: Skip Step 5 (Feature Binding) ===
        # Use slots directly for decoding
        bound_features = slots
        enhanced_features = None  # Not used in spatial variant
