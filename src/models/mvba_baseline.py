...
# Alpha generator, spatial binding, and feature binding are disabled for baseline
...
class MVBA(nn.Module):
    ...
    def __init__(
        ...
        # Baseline variant: Alpha generator, spatial binding, and feature binding are disabled
    ...
    def forward(
        ...
        # === Baseline: Skip Steps 3-5 (No Alpha, Spatial, or Feature Binding) ===
        # Use slots directly for decoding
        bound_features = slots  # Use raw slot representations
        
        # Create placeholder values for compatibility
        B = images.shape[0]
        H, W = features.shape[2:]
        alphas = {
            'spatial': torch.ones(B, self.n_slots, 1, device=images.device),
            'feature': torch.ones(B, self.n_slots, 1, device=images.device)
        }
        # Spatial attention will be set from decoder masks later
        enhanced_features = None  # Not used in baseline
        ...
        for slot_idx in range(self.n_slots):
            ...
            # For baseline, we use slots directly (bound_features = slots)
        ...
        # For baseline, use decoder masks as spatial attention
